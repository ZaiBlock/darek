...
client.on('messageCreate', async (message) => {
  ...
  try {
    // Al no usar await aquí evitamos bloquear otras ejecuciones si llegan mensajes simultáneos
...
`;
    ...
    if (stateMatch) {
      ...
      try {
        ...
        // Actualizar Presencia en Discord a modo Burbuja (Estado Personalizado)
        if (botState.status || botState.customStatusText) {
          client.user.setPresence({
            status: botState.status || 'online',
            activities: [{ 
              name: 'Custom Status', 
              type: ActivityType.Custom,
              state: botState.customStatusText || 'pensando...'
            }]
          });
        }
    ...
    // Actualizar historial local por usuario de manera atómica post-respuesta
